...
```{r}
f = 
  function(x, eager=FALSE) if(eager) eval(x) + 1 else {z = match.call(); z$eager = T;z}
f(2)
f(2, F)
f(2, T)
eval(f(2))
...
```{r}
lazy = function(f) function(eager=FALSE, ...) if(eager) do.call(f, lapply(list(...), eval)) else {z = match.call(); z$eager = T;z}
g = function(x) x + 1
h = lazy(g)
h(T,1)
h(T,2)
